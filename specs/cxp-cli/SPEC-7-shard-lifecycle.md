...
**Status:** Draft
**Depends on:** SPEC-0 (CLI skeleton), SPEC-2 (metadata), SPEC-5 (unified search — `cxp shard` namespace, `create_edge()`, labels table)
**Blocks:** Nothing
...
4. **`cxp epic create`** — create an epic with children or adopt existing shards
5. **`cxp epic show`** — epic detail with progress bar and child status
6. **`cxp epic list`** — list all epics with completion stats
7. **`cxp shard assign`** — claim a shard (set owner + in_progress)
8. **`cxp shard close`** — close a shard with reason
9. **`cxp shard next`** — find next unblocked shard (within focus or globally)
10. **`cxp shard board`** — kanban view of shards by status
11. **`cxp focus`** — show/set/clear active epic
...
| `kind:epic` | Epic container | `cxp epic create` or Phase 3 |

### Data Flow

#### Epic shard (type='epic')

1. **WHO writes it?** `cxp epic create` or ingest Phase 3 (when decomposing HIGH items)
...
4. **WHO reads it?** `cxp epic show`, `cxp epic list`, `cxp shard board`, `cxp shard next`, `cxp focus`
...
1. **WHO writes it?** `cxp focus set` or agent startup
2. **WHEN is it written?** When user/agent declares current work area
3. **WHERE is it stored?** `focus` table (project, agent, epic_id)
4. **WHO reads it?** `cxp focus`, `cxp shard next`, `cxp shard board`, assistant pickup
...
1. **WHO writes it?** `cxp shard assign` (sets owner + in_progress), `cxp shard close` (sets closed), agents via pipeline
...
4. **WHO reads it?** `cxp shard board`, `cxp shard next`, `cxp epic show`, humans asking "what's mycroft doing?"
...
1. **WHO writes it?** Ingest Phase 1 (classify) for incoming work. `cxp epic create` for epics. Manual via `cxp shard label add <id> kind:bug`.
...
### `cxp epic create` — Create Epic

```bash
# Create epic with title and description
cxp epic create --title "Pipeline Quality" \
...
cxp epic create --title "Pipeline Quality" --body "..." --priority 2

# Set ordering (blocked-by edges) during creation
cxp epic create --title "Pipeline Quality" --body "..." \
    --adopt pf-25f2f3,pf-5d3921,pf-1296a9 \
...
### `cxp epic show` — Epic Detail with Progress

```bash
cxp epic show <epic-id>
cxp epic show <epic-id> -o json
```
...
```bash
cxp epic list
cxp epic list --status open         # only open epics (default)
cxp epic list --status all          # include closed epics
cxp epic list -o json
```
...
### `cxp shard assign` — Claim a Shard

```bash
cxp shard assign <shard-id> --agent agent-mycroft
cxp shard assign <shard-id>    # uses configured agent from cxp config
```
...
### `cxp shard close` — Close a Shard

```bash
cxp shard close <shard-id> --reason "Done: implemented and tested"
cxp shard close <shard-id>   # no reason (allowed, but reason is recommended)
```
...
### `cxp shard next` — Next Workable Shard

```bash
cxp shard next                     # within focused epic (if set), else all open
cxp shard next --epic pf-abc123    # within specific epic
cxp shard next --global            # ignore focus, search all open shards
cxp shard next -o json
```
...
### `cxp shard board` — Kanban View

```bash
cxp shard board                     # within focused epic
cxp shard board --epic pf-abc123    # specific epic
cxp shard board --global            # all open work
cxp shard board --agent agent-mycroft  # what's this agent working on?
cxp shard board -o json
```
...
### `cxp focus` — Show/Set/Clear Active Epic
...
# Set focus
cxp focus set <epic-id>
cxp focus set <epic-id> --note "working on this until EOD"
...
**`cxp focus` (show) — What it does:**
1. Call `focus_get()` for current agent+project.
2. If no focus set: "No focus set. Use `cxp focus set <epic-id>` to set one."
...
**`cxp focus set` — What it does (atomic):**
...
Agent receives work (ingest pipeline or manual)
            ...
            ▼
  ┌────────────────────┐
  │  cxp shard next     │  ← scoped to focused epic
  │  → returns pf-xxx  │
  └────────┬───────────┘
           │
           ▼
  ┌────────────────────┐
  │  cxp shard assign   │  ← sets in_progress + owner
  │  pf-xxx            │
  └────────┬───────────┘
           │
           ▼
  ...
  └────────┬───────────┘
           ...
           ▼
  ┌────────────────────┐
  │  cxp shard close    │  ← sets closed + reason
  │  pf-xxx "Done..."  │  ← reports unblocked shards
  └────────┬───────────┘
           │
           ▼
  ┌────────────────────┐
  │  cxp shard next     │  ← loop: next shard
  └────────────────────┘
```
...
1. Create an epic shard: `cxp epic create --title "feat: [feature name]"`
...
```bash
cxp shard close <shard-id> "Verified: all tests pass, deployed as commit abc123"
```
After closing all layer sub-shards, close the parent epic if all children are done:
```bash
# Check if epic is complete
cxp epic show <epic-id> -o json  # check progress.open == 0 && progress.in_progress == 0
cxp shard close <epic-id> "All shards complete"
```
...
1. **Epic creation** — `cxp epic create` produces a type='epic' shard. `--adopt` re-parents existing shards. `--order` creates `blocked-by` edges. Adopted shards appear as children in `cxp epic show`.
2. **Epic progress** — `cxp epic show` displays accurate completion stats computed from children's status. Progress bar reflects completed/total ratio.
3. **Kind labels** — All work shards have a `kind:X` label. `cxp shard board` and `cxp epic show` display kind per shard.
4. **Shard assign** — `cxp shard assign` atomically sets status=in_progress + owner. Rejects if already assigned, closed, or blocked.
5. **Shard close** — `cxp shard close` atomically sets status=closed with reason. Reports newly unblocked shards. Idempotent on already-closed shards.
6. **Shard next** — `cxp shard next` returns unblocked open shards ordered by priority. Respects focus scope. Returns empty when all work is done or blocked.
7. **Board view** — `cxp shard board` groups shards by status (open/in_progress/completed). Shows owner and time for in_progress shards. Shows blocked-by for blocked shards.
8. **Focus persistence** — `cxp focus set` survives context clears. `cxp focus` shows current epic with progress. `cxp focus clear` removes focus. Auto-clears if epic is closed.
...
12. **Agent visibility** — `cxp shard board --agent X` shows what agent X is working on and recently completed.
...
TEST: board view shows kanban columns
  ...
  When:  cxp shard board --epic <epic-id>
...
TEST: agent filter on board
  ...
  When:  cxp shard board --agent agent-mycroft
  Then:  Only shows pf-a

TEST: focus survives context clear
  Given: cxp focus set <epic-id>
  When:  New session (context cleared), run cxp focus
...
TEST: shard close on last child does not auto-close epic
  ...
  When:  cxp shard close pf-a "Done"
  Then:  pf-a closed. Epic E still open. cxp epic show shows 1/1 complete but epic status = open.
```
...
| Phase 4 (Implement) | Launches agents, no assignment tracking | Calls `cxp shard assign <id>` before starting work |
| Phase 5 (Verify) | Calls `/palace task close` inconsistently | Calls `cxp shard close <id> "reason"` for each verified shard + parent epic |
...
| **SPEC-5** (unified search) | **Supersedes** SPEC-5's `cxp shard close` (adds `closed_by`, `closed_reason`, unblocked reporting). SPEC-5's `cxp shard reopen` remains valid — it resets status to `open` and clears `closed_at`/`closed_by`/`closed_reason`. Uses `cxp shard` namespace, `create_edge()`, and labels table from SPEC-5. |

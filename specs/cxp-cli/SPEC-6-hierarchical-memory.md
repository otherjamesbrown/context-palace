...
- SPEC-5 `cxp memory` — labels, references, semantic recall (flat)
- No hierarchy, no sub-memories, no access tracking

## What to Build

1. **Sub-memory pointer block** — structured JSON section in parent content
2. **`cxp memory add-sub`** — create child + AI-generated summary + update parent
3. **`cxp memory delete`** — remove shard + remove pointer from parent (atomic)
4. **`cxp memory move`** — re-parent a memory (update old parent, new parent, shard)
5. **`cxp memory promote`** — move a child up one level
6. **`cxp memory show --depth N`** — expand children inline
7. **`cxp memory tree`** — full hierarchy view with access stats
8. **`cxp memory sync`** — reconcile pointer blocks from graph (safety net)
9. **`cxp memory hot`** — most-accessed memories by depth (promotion candidates)
...
interface. The edge metadata enables efficient tree queries. `cxp memory sync` reconciles
both if they drift.
...
When `cxp memory show` displays the memory, the pointer block is rendered as a
readable list:
...
Every `cxp memory show` call increments telemetry counters in the shard's metadata:
...
1. **WHO writes it?** `cxp memory add-sub` (sets on child creation), `cxp memory move` (updates), `cxp memory promote` (updates)
2. **WHEN is it written?** On child creation, move, or promote.
3. **WHERE is it stored?** `shards.parent_id` column (indexed FK)
4. **WHO reads it?** `memory_tree()`, `memory_children()`, `memory_path()`, `memory_hot()`, `cxp memory sync`, `cxp memory delete` (to find parent for pointer removal)
...
7. **DOES it go stale?** No — updated atomically in transactions. If a parent shard is deleted externally, `parent_id` becomes a dangling FK. `cxp memory sync` detects this and can fix it.

#### Sub-memory pointer block (in shard content)

1. **WHO writes it?** `cxp memory add-sub` (appends entry), `cxp memory delete` (removes entry), `cxp memory move` (removes from old parent, adds to new), `cxp memory promote` (same as move), `cxp memory sync` (reconciles)
...
4. **WHO reads it?** `cxp memory show` (renders as readable list), `cxp memory sync` (validates), AI agents (read pointers in-context to decide what to load).
...
7. **DOES it go stale?** Yes — if edges/parent_id are modified without updating the pointer block (e.g., external SQL manipulation). `cxp memory sync` is the safety net.

#### Summary in edge metadata (child-of edge)

1. **WHO writes it?** `cxp memory add-sub` (creates edge with summary), `cxp memory move` (deletes old edge, creates new with same summary), `cxp memory promote` (same as move), `cxp memory sync` (reconciles).
...
4. **WHO reads it?** `memory_tree()` SQL function (LEFT JOIN edges), `cxp memory tree`, `cxp memory hot`.
...
7. **DOES it go stale?** Can drift from the pointer block if one is updated without the other. Both are always written in the same transaction, so drift only occurs from external SQL manipulation. `cxp memory sync` reconciles both.

#### Access telemetry (in shard metadata)

1. **WHO writes it?** `memory_touch()` SQL function, called by `cxp memory show`.
2. **WHEN is it written?** On every `cxp memory show` call (including when expanded via `--depth`).
3. **WHERE is it stored?** `shards.metadata` JSONB: `access_count` (int), `last_accessed` (timestamp), `access_log` (array of {at, by, depth}).
4. **WHO reads it?** `cxp memory tree --stats`, `cxp memory hot`, `cxp memory show` (displays count in header).
...
the reader's `cxp memory show` may fail with "not found." This is expected behavior.
...
**Config** (in `~/.cxp/config.yaml`, alongside the existing `embedding:` section):
...
```
User runs: cxp memory add-sub pf-aa1 --title "Troubleshooting" --body-file troubleshoot.md
                                │
                                ▼
                    ┌──────────────────────┐
                    │  1. Read child body   │
...
cxp memory add-sub pf-aa1 --title "Troubleshooting" \
...
### `cxp memory add-sub` — Create Sub-Memory

```bash
# Interactive (AI-assisted)
cxp memory add-sub <parent-id> --title "Troubleshooting" \
...
### `cxp memory delete` — Delete Memory

```bash
cxp memory delete <id>

# Force delete (skip confirmation)
cxp memory delete <id> --force

# Delete including all children (recursive)
cxp memory delete <id> --recursive
```
...
### `cxp memory move` — Re-Parent Memory

```bash
cxp memory move <id> <new-parent-id>

# Move to root (no parent)
cxp memory move <id> --root
```
...
### `cxp memory promote` — Move Up One Level

```bash
cxp memory promote <id>
```
...
- Equivalent to `cxp memory move <id> <grandparent-id>` or `cxp memory move <id> --root`
...
### `cxp memory show` — Show Memory (New Command)

**Note:** This is a new command introduced by SPEC-6. There is no `cxp memory show` in
SPEC-5 (SPEC-5 provides `cxp shard show` for generic shard viewing). `cxp memory show` is
memory-specific: it renders the sub-memories pointer block, tracks access telemetry, and
supports `--depth` expansion. For non-memory shards, use `cxp shard show`.
...
cxp memory show <id> --depth 1    # show + immediate children content
cxp memory show <id> --depth 2    # show + children + grandchildren

# JSON output
cxp memory show <id> -o json
cxp memory show <id> --depth 1 -o json
```
...
### `cxp memory tree` — Hierarchy View
...
### `cxp memory hot` — Promotion Candidates
...
### `cxp memory sync` — Reconcile Pointer Blocks
...
cxp memory sync --dry-run

# Fix all parents
cxp memory sync

# Fix specific parent
cxp memory sync <parent-id>
```
...
### `cxp memory add` — Create Root Memory (unchanged from SPEC-5)

```bash
# Existing behavior, unchanged
cxp memory add "Deployment overview for Penfold system..." \
    --label deployment,infrastructure
```

Root memories created with `cxp memory add` have no sub-memories block initially.
The block is created automatically when the first `add-sub` call adds a child.

### `cxp memory list` — Flat by Default (amended in SPEC-5)

`cxp memory list` shows all memories (root and child) in a flat list by default.
...
# Root memories only
cxp memory list --roots

# Hierarchical view (SPEC-6)
cxp memory tree
```

The `--roots` flag is added to SPEC-5's `cxp memory list` flag table. The full
hierarchical view is `cxp memory tree` (this spec).

## Database Changes

### Migration File

All SQL changes in this spec go into `cxp/migrations/007_hierarchical_memory.sql`.
...
12. **Semantic search finds children:** `cxp memory recall` and `cxp recall` find
...
14. **`cxp memory list --roots`:** (SPEC-5 amendment) Shows only memories with `parent_id IS NULL`.
    Default `cxp memory list` shows all memories (root and child) in flat list.
...
TEST: add-sub creates child with pointer
  ...
  When:  `cxp memory add-sub pf-aa1 --title "Troubleshooting" --body "..." --no-ai --summary "If deploy fails"`
  Then:  New shard created with parent_id = pf-aa1
         `cxp memory show pf-aa1` shows sub-memories block with entry
         `cxp shard edges <child-id>` shows child-of edge to pf-aa1

TEST: add-sub with AI generates summary
  Given: Root memory with deployment content
  When:  `cxp memory add-sub pf-aa1 --title "Troubleshooting" --body-file troubleshoot.md --auto-approve`
...
TEST: add-sub creates first block
  ...
  When:  `cxp memory add-sub pf-aa1 --title "X" --body "Y" --no-ai --summary "Z"`
...
TEST: add-sub appends to existing block
  ...
  When:  `cxp memory add-sub pf-aa1 --title "Third" --body "..." --no-ai --summary "S"`
  Then:  Block now has 3 entries, original 2 preserved

TEST: add-sub with labels
  Given: Root memory pf-aa1
  When:  `cxp memory add-sub pf-aa1 --title "X" --body "Y" --no-ai --summary "Z" --label deployment,nomad`
...
TEST: delete leaf memory
  ...
  When:  `cxp memory delete B --force`
...
TEST: delete root memory (no parent)
  ...
  When:  `cxp memory delete A --force`
...
TEST: show non-existent memory
  ...
  When:  `cxp memory show pf-nonexistent`
  Then:  Exit code 1, error "not found"

TEST: show --depth 6 rejected
  Given: Root memory A
  When:  `cxp memory show A --depth 6`
...
TEST: hot min-depth filter
  ...
  When:  `cxp memory hot --min-depth 2`
  Then:  Only C shown
```
...
TEST: recall finds sub-memories
  ...
  When:  `cxp recall "deployment issues"`
...
TEST: recall finds deeply nested memory
  ...
  When:  `cxp recall "nomad allocation"`
...
TEST: add-sub deep nesting warning
  ...
  When:  `cxp memory add-sub F --title "G" --body "..." --no-ai --summary "S"`
...
| **SPEC-1** (semantic search) | Sub-memories get embeddings on create. `cxp recall` and `cxp memory recall` find sub-memories regardless of tree position. |
...
| **SPEC-5** (unified search) | Extends `cxp memory` from SPEC-5 with `add-sub`, `delete`, `move`, `promote`, `tree`, `hot`, `sync`, enhanced `show`. Uses `create_edge()` (SPEC-5) for `child-of` edges. `cxp memory add` (SPEC-5) creates root memories. `cxp memory list` (SPEC-5) amended with `--roots` flag to filter to root-only. `parent_id` column is existing infrastructure used by SPEC-6 for the first time. |
...
3. **`cxp memory list` default:** Shows all memories (root and child) in flat list. `--roots`
   flag added to SPEC-5 to filter to root-level only. `cxp memory tree` for hierarchical view.
...
8. **Summary storage (dual write):** Summaries stored in both the parent's pointer block (for
   ...
   display). `memory_tree()` LEFT JOINs edges to return summaries. `cxp memory sync` reconciles both.

...
- `cxp backlog` — basic work items without structured criteria or lifecycle
...
6. **`cxp requirement` commands** — full CRUD + lifecycle + linking
...
```bash
# Create
cxp requirement create "Entity Lifecycle Management" \
    --priority 2 \
...
# List with filters
cxp requirement list --status approved
cxp requirement list --category entity-management
cxp requirement list --status draft,approved   # multiple statuses

# Show detail
cxp requirement show pf-req-01
# Output:
...
cxp requirement approve pf-req-01
cxp requirement verify pf-req-01
cxp requirement reopen pf-req-01 --reason "test failures found"

# Link implementation task
cxp requirement link pf-req-01 --task pf-task-123
# Creates edge: pf-task-123 --implements--> pf-req-01

# Link test
cxp requirement link pf-req-01 --test pf-test-789

# Link dependency
cxp requirement link pf-req-05 --depends-on pf-req-03

# Unlink
cxp requirement unlink pf-req-01 --task pf-task-123

# Dashboard
cxp requirement dashboard
# Output:
...
# JSON output
cxp requirement list -o json
cxp requirement dashboard -o json
```

## Success Criteria

1. **Create:** `cxp requirement create` creates shard with type=requirement, metadata
...
TEST: create requirement
  When:  `cxp requirement create "Test Req" --priority 2 --category testing`
  Then:  Exit code 0
         Output contains shard ID
         `cxp requirement show <id>` shows status=draft, priority=2, category=testing

TEST: approve requirement
  Given: Requirement in draft status
  When:  `cxp requirement approve <id>`
  Then:  Exit code 0
         `cxp requirement show <id>` shows status=approved
...
TEST: link task triggers in_progress
  ...
  When:  `cxp requirement link <req-id> --task <task-id>`
  Then:  Edge created
...
TEST: reopen verified
  ...
  When:  `cxp requirement reopen <id> --reason "found regression"`
  Then:  Status = approved
         metadata.reopen_reason = "found regression"

TEST: reopen without reason fails
  Given: Requirement in verified status
  When:  `cxp requirement reopen <id>`
  Then:  Exit code 1

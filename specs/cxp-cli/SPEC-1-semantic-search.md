...
- `cxp memory search "query"` — text search, memory type only
...
5. **`cxp recall` command** — semantic search CLI
...
### On shard create/update (in `cxp` CLI)
...
```bash
cxp admin embed-backfill
# Embedding 587 shards... [=====>    ] 312/587
# Rate: 50/min | ETA: 5m 30s
# Done: 587 embedded, 0 failed

# Options
cxp admin embed-backfill --dry-run          # Show count, don't embed
cxp admin embed-backfill --batch-size 10    # Shards per API call (if batching supported)
cxp admin embed-backfill --type task        # Only embed task shards
```
...
# Filter by label
cxp recall "CLIC" --label architecture
...
2. **Embedding on create:** `cxp shard create` embeds content within 5 seconds.
3. **Embedding on update:** `cxp shard update` (content change) regenerates embedding.
...
8. **Backfill:** `cxp admin embed-backfill` embeds all existing shards.
...
| No embedding config | Error: "No embedding provider configured. Add `embedding:` section to ~/.cxp/config.yaml" |
| Embedding dimension mismatch | Error: "Existing embeddings are 768-dim, provider returns 1536-dim. Run `cxp admin embed-reset`." |
...
TEST: create shard embeds content
  ...
  When:  `cxp shard create --type memory --title "Timeout lesson" --body "AI client timeout is 120s"`
...
TEST: embed-backfill dry-run
  Given: 10 shards without embeddings
  When:  `cxp admin embed-backfill --dry-run`
...
TEST: shard update re-embeds
  ...
  When:  Content is updated via `cxp shard update`
...
TEST: recall without embedding config falls back
  ...
  Then:  Error: "Semantic search requires embedding config."
         Suggests: "Use `cxp shard list --search 'query'` for text search."
```

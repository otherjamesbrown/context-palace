...
func TestLoadConfig_EnvOverride(t *testing.T) {
    os.Setenv("CXP_HOST", "localhost")
    defer os.Unsetenv("CXP_HOST")
...
func TestLoadConfig_ProjectFile(t *testing.T) {
    // Write temp .cxp.yaml, verify it overrides global config
    // ...
}
```
...
**Framework:** Go test with `os/exec` to run the `cxp` binary, or test the command
...
```
context-palace/
├── palace/                    # Existing (to be replaced by cxp/)
├── cxp/                        # New cxp CLI
│   ├── main.go
│   ├── go.mod
│   ├── cmd/

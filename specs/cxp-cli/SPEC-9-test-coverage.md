...
- 49 Go source files across 7 packages (cxp module) + 5 files (palace module)
...
import (
    ...
    "github.com/otherjamesbrown/context-palace/cxp/internal/pointer"
...
TEST: LoadConfig_EnvOverrides
  Given: config file with host=filehost, env CXP_HOST=envhost
...
TEST: LoadConfig_ProjectOverrides
  Given: global config with agent=A, .cxp.yaml with agent=B
  When:  LoadConfig (from dir with .cxp.yaml)
  Then:  config.Agent == "B"

TEST: LoadConfig_AllEnvVars
  Given: CXP_HOST, CXP_DATABASE, CXP_USER, CXP_PROJECT, CXP_AGENT all set
  When:  LoadConfig("")
...
1. `go test ./...` runs from `cxp/` directory and passes with zero failures
...
TEST: LoadConfig_EnvOverrides
  Given: config file + CXP_HOST env var

...
**Blocks:** SPEC-6 (hierarchical memory extends `cxp memory`)

---

## Goal

Bring everything together. General shard operations (`cxp shard`), semantic search
across all types (`cxp recall`), edge navigation, label management, and enhanced agent
...
1. **`cxp shard` commands** — general CRUD for any shard type, edge browsing, label ops
2. **`cxp recall`** — semantic search across all shard types (signature command)
3. **Enhanced `cxp memory`** — labels, edges to related shards, semantic recall
...
1. **WHO writes it?** Go CLI code after `create_shard()` or `cxp shard update` succeeds
...
1. **WHO writes it?** `cxp shard link`, `cxp requirement link` (SPEC-3), `cxp knowledge update` (SPEC-4, for previous-version), `cxp memory add --references`, `cxp memory add-sub` (SPEC-6, for child-of)
...
4. **WHO reads it?** `cxp shard edges`, `cxp shard show` (edge summary), `shard_edges()`, `has_circular_dependency()` (SPEC-3), `knowledge_history()` (SPEC-4), `memory_tree()` (SPEC-6)
...
1. **WHO writes it?** `cxp shard create --label`, `cxp shard label add/remove`, `cxp memory add --label`
...
4. **WHO reads it?** `cxp shard list --label`, `cxp shard show`, `cxp shard labels`, `list_shards()`, `label_summary()`, `semantic_search()` (SPEC-1 label filter)
...
`cxp shard update` — it's a simple UPDATE. Knowledge docs have their own locking via
SPEC-4's `FOR UPDATE`.
...
cxp recall "timeout" --status open
cxp recall "old decisions" --include-closed

# Adjust similarity threshold (default 0.3)
cxp recall "vague query" --min-similarity 0.5

# Since filter (content created after date)
cxp recall "deployment" --since 7d
cxp recall "architecture" --since 2026-01-01

# Limit results (default 20)
cxp recall "deployment" --limit 5

# JSON output for programmatic use
cxp recall "entity management" -o json --limit 5
...
3. Embed the query text using configured embedding provider (SPEC-1). Error if no provider configured: "Semantic search requires embedding config. Use `cxp shard list --search` for text search."
...
### `cxp shard list` — List shards with filters

```bash
cxp shard list
cxp shard list --type task --status open
cxp shard list --type requirement,bug --status open --since 7d
cxp shard list --creator agent-mycroft --limit 50
cxp shard list --label architecture
cxp shard list --search "timeout"    # text search (tsvector, not semantic)
```
...
| `--status` | No | all statuses | Comma-separated statuses. Note: unlike `cxp recall`, no default status filter |
| `--label` | No | — | Comma-separated labels (OR/overlap) |
...
2. Convert `--since` to `TIMESTAMPTZ` cutoff (same as `cxp recall`).
...
**Design note:** `cxp shard list` defaults to ALL statuses (no filter), unlike `cxp recall`
...
### `cxp shard show` — Show shard detail

```bash
cxp shard show pf-c74eea
cxp shard show pf-c74eea -o json
```
...
### `cxp shard create` — Create a shard
...
### `cxp shard update` — Update shard content or title

```bash
# Update content from inline
cxp shard update pf-abc123 --body "Updated content"
...
# Update both
cxp shard update pf-abc123 --title "New Title" --body "New content"
```
...
5. If shard `type = 'knowledge'`, print warning: "This is a knowledge document. Use `cxp knowledge update` to preserve version history. Updating directly."
...
**Note:** Metadata updates go through `cxp shard metadata set` (SPEC-2). This command
...
### `cxp shard close` / `cxp shard reopen` — Status management

```bash
cxp shard close pf-abc123
cxp shard reopen pf-abc123
cxp shard close pf-abc123 -o json
```
...
### `cxp shard edges` — Edge Navigation

```bash
# Show all edges for a shard
cxp shard edges pf-abc123
...
# JSON output
cxp shard edges pf-abc123 -o json
```
...
### `cxp shard link` / `cxp shard unlink` — Edge Management
...
# Remove edges
cxp shard unlink pf-task-123 --implements pf-req-01
...
**Note on SPEC-3 lifecycle interaction:** `cxp shard link --implements` creates the edge
but does NOT trigger SPEC-3's automatic lifecycle transition (approved → in_progress).
That transition only happens via `cxp requirement link --task`, which is the lifecycle-aware
command. `cxp shard link` is the low-level graph tool. If lifecycle automation is desired,
use `cxp requirement link --task` instead.
...
### `cxp shard label` — Label Management

```bash
# Add labels to a shard
cxp shard label add pf-abc123 architecture pipeline

# Remove labels
cxp shard label remove pf-abc123 pipeline

# List all labels in use (discovery)
cxp shard label list
# Output:
#   LABEL           COUNT
#   architecture    12
#   pipeline        8
...
**Naming note:** The subcommand is `cxp shard label` (singular) with verbs `add`, `remove`,
`list`. This is consistent with the `cxp shard link`/`unlink` pattern and avoids confusion
...
cxp memory add "Nomad deploys are unreliable — always verify with version check"

# Add memory with labels
cxp memory add "AI client timeout was hardcoded at 120s" \
    --label timeout,pipeline,lesson-learned

# Add memory with edge links
cxp memory add "Entity display names missing because NER stage doesn't extract them" \
...
cxp memory search "timeout"

# Semantic recall (NEW — uses embedding)
cxp memory recall "deployment issues"

# Resolve/defer (same as penf)
cxp memory resolve pf-mem-12
cxp memory defer pf-mem-08 --until 2026-02-14
```

#### `cxp memory add` — Add a memory
...
show only root-level memories. See SPEC-6 `cxp memory tree` for hierarchical views.

#### `cxp memory recall` — Semantic search over memories
...
1. Embed query using configured provider. Error if no provider: "Semantic recall requires embedding config. Use `cxp memory search` for text search."
...
#### `cxp memory search`, `resolve`, `defer` — Penf-compatible commands

These commands are identical to their `penf memory` equivalents:

- **`cxp memory search <query>`** — Full-text search (tsvector) over memory shards. Same as `penf memory search`.
- **`cxp memory resolve <id>`** — Close a memory shard (sets status=closed). Same as `penf memory resolve`.
- **`cxp memory defer <id> --until <date>`** — Set a `deferred_until` metadata field. Same as `penf memory defer`.
...
no references, text search only). `cxp memory` is the enhanced version with labels,
references, semantic recall. Both operate on the same `type='memory'` shards in the database.
Memory shards created via `cxp shard create --type memory` are plain memory shards —
use `cxp memory add` for the standard creation flow with labels/references, and
`cxp memory add-sub` (SPEC-6) for hierarchical memories.
...
### `cxp shard link` Workflow

```
cxp shard link <from> --<edge-type> <to>
    │
    ▼
...
### `cxp shard create` Workflow

```
cxp shard create --type <type> --title <title> [--body|--body-file] [flags]
    │
    ▼
...
-- labels don't appear. Use cxp shard list --label X --status closed to find
...
cxp/
├── cmd/
│   ├── recall.go               # cxp recall
│   ├── shard.go                # cxp shard list/show/create/update/close/reopen
│   ├── shard_edges.go          # cxp shard edges/link/unlink
│   ├── shard_label.go          # cxp shard label add/remove/list
│   └── memory.go               # Enhanced cxp memory (add --label, recall)
└── internal/
    └── client/
...
```go
// RecallOpts holds all flags for cxp recall
...
1. **`cxp shard list`:** Lists any shard type with --type, --status, --label, --creator,
   --since, --search filters. Pagination via --limit and --offset. Total count shown.
2. **`cxp shard show`:** Shows content, metadata, labels, and full edge list in one view.
   JSON output includes edges inline.
3. **`cxp shard create`:** Creates shards of any type with content, labels, metadata.
   Generates embedding (SPEC-1). Warns on unknown type. Requires --type and --title.
4. **`cxp shard update`:** Updates content and/or title. Regenerates embedding on content
   change. Warns when updating knowledge docs (use `cxp knowledge update` instead).
5. **`cxp shard edges`:** Shows all incoming and outgoing edges with linked shard info.
   Filters by direction and edge type.
6. **`cxp shard edges --follow`:** Tree view of N-hop edge navigation (default 2, max 5).
   Detects cycles and marks revisited shards.
7. **`cxp shard link`:** Creates typed edges between shards. Validates both exist, rejects
   self-reference, rejects circular blocked-by, prevents duplicates via UNIQUE constraint.
8. **`cxp shard unlink`:** Removes edges. Confirms before removing unless `--force`.
   Non-interactive mode requires `--force`.
9. **`cxp shard label add/remove`:** Adds/removes labels atomically (SQL array ops, no
   read-modify-write). Duplicate add is no-op. Missing remove is no-op.
10. **`cxp shard label list`:** Lists all labels in use with counts (excludes closed shards).
11. **`cxp recall`:** Semantic search across all types. Ranked by similarity. `--since`
    filter applied in SQL before LIMIT. Supports all filter flags.
12. **`cxp memory add --label`:** Memory shards with labels on create.
13. **`cxp memory add --references`:** Creates reference edges from memory to other shards.
    Missing reference targets produce warning, not error.
14. **`cxp memory recall`:** Semantic search limited to memory type.
...
|------|-------------------|
| `cxp shard list` no filters | All shards (any status), newest first, limit 20. |
| `cxp shard list` no results | Empty table, exit code 0. |
| `cxp shard show` non-existent ID | Error: "Shard pf-xxx not found." Exit code 1. |
| `cxp shard show` on message shard | Works. Unified interface for all types. |
| `cxp shard show` on version snapshot (e.g. pf-arch-001-v2) | Works. All shards treated equally regardless of origin. |
| `cxp shard create` without --type | Error: "--type is required." |
| `cxp shard create` without --title | Error: "--title is required." |
| `cxp shard create` unknown type | Warn: "Type 'foo' is not a known type. Create anyway? (y/n)." In non-interactive mode, reject. |
| `cxp shard create` with both --body and --body-file | Error: "Cannot use both --body and --body-file." |
| `cxp shard create` with --body-file non-existent | Error: "Cannot read file 'missing.md': no such file or directory" |
| `cxp shard create` with empty body | Allowed. Shard created with empty content. Embedding uses title only (SPEC-1). |
| `cxp shard update` on knowledge shard | Warning: "This is a knowledge document. Use `cxp knowledge update` to preserve version history." Update proceeds (low-level tool). |
| `cxp shard update` with no update flags | Error: "At least one of --body, --body-file, or --title is required." |
| `cxp shard close` already closed | No-op: "Shard pf-abc123 is already closed." |
| `cxp shard reopen` already open | No-op: "Shard pf-abc123 is already open." |
| `cxp recall` very short query (1 word) | Works but may have low discrimination. |
| `cxp recall` no results above threshold | Empty result, exit code 0. "No results above 0.3 similarity." |
| `cxp recall` without embedding config | Error: "Semantic search requires embedding config. Use `cxp shard list --search` for text search." |
| `cxp recall --status` and `--include-closed` both set | Error: "--status and --include-closed are mutually exclusive." |
| `cxp recall --min-similarity 1.5` | Error: "min-similarity must be between 0.0 and 1.0" |
| `cxp shard link` self-reference | Error: "Cannot create edge from a shard to itself." |
| `cxp shard link` to non-existent shard | Error: "Shard pf-xxx not found." |
| `cxp shard link` duplicate edge | Error: "Edge already exists: pf-a --implements--> pf-b." |
| `cxp shard link` unknown edge type | Error: "Unknown edge type: foo. Valid: blocked-by, implements, references, ..." |
| `cxp shard link --blocked-by` circular | Error: "Circular dependency detected." |
| `cxp shard unlink` non-existent edge | Error: "No edge of type 'implements' from pf-a to pf-b." |
| `cxp shard unlink` without --force in pipe | Error: "Use --force for non-interactive unlink." |
| `cxp shard label add` duplicate label | No-op for that label. Labels are a set. |
| `cxp shard label remove` non-existent label | No-op. |
| `cxp shard label list` no labels in project | Empty table, exit code 0. |
| `cxp shard edges --follow` with cycle | Shows each shard at most once. Marks revisited: "(cycle: pf-xxx already shown)". |
| `cxp shard edges --follow --max-depth 10` | Error: "max-depth must be 1-5." |
| `cxp memory add --references` invalid ID | Warning: "Shard pf-xxx not found. Memory created without edge." |
| `cxp memory recall` no embedding config | Error: "Semantic recall requires embedding config. Use `cxp memory search` for text search." |
| `--since` with bad format | Error: "Invalid duration: '7x'. Use format like '7d', '24h', or '2026-01-01'." |
| `cxp shard list --type task,bug` | Returns shards of either type (OR within filter). |
| `cxp shard list --label a,b` | Returns shards with ANY of those labels (OR / overlap). |
...
| `cxp shard delete` (no such command) | Intentionally omitted. Soft-delete via `cxp shard close` is the pattern. Hard delete deferred to a future admin spec. Orphaned edges from externally-deleted shards are silently excluded by JOIN in `shard_edges()`. |
| Large result set | Paginate: "Showing 1-20 of 347 results." |
| `cxp shard create` embedding fails | Shard created, warning logged, embedding NULL. |
| `cxp shard update` embedding fails | Content updated, warning logged, embedding stale. |
...
| **SPEC-1** (semantic search) | `cxp recall` wraps `semantic_search()`. Embedding generated on shard create/update. **Amendment included above:** `p_since TIMESTAMPTZ` parameter added to `semantic_search()` for correct `--since` + `--limit` behavior. Note: `semantic_search()` defaults `p_status` to NULL (all statuses), but `cxp recall` overrides this to `["open"]` in Go. |
| **SPEC-2** (metadata) | Metadata read/written via SPEC-2's `update_metadata()` and GIN index. `cxp shard show` displays metadata. Metadata updates go through `cxp shard metadata set` (SPEC-2), not `cxp shard update`. |
| **SPEC-3** (requirements) | `cxp shard link --implements` creates the edge but does NOT trigger SPEC-3's lifecycle auto-transition (approved → in_progress). Use `cxp requirement link --task` for lifecycle-aware linking. `cxp shard link --blocked-by` uses `has_circular_dependency()` from SPEC-3. |
| **SPEC-4** (knowledge docs) | `cxp shard update` on type=knowledge bypasses versioning. Warning printed. Knowledge version snapshots (closed shards) are visible via `cxp shard show`. |
| **SPEC-5** defines the edge UNIQUE constraint and `create_edge()`/`delete_edge()` functions used by all specs that create edges. |
| **SPEC-6** (hierarchical memory) | Extends `cxp memory` with `add-sub`, `tree`, `show --depth`, etc. Memory shards created via `cxp shard create --type memory` are plain memories (no pointer blocks). SPEC-6 commands (`cxp memory add-sub`, `cxp memory show --depth`) handle hierarchical structure. |
...
TEST: parseRecallFlags defaults
  Given: `cxp recall "query"` (no flags)
...
TEST: recall --include-closed shows closed
  ...
  When:  `cxp recall "timeout"`
  Then:  Only open bug
  When:  `cxp recall "timeout" --include-closed`
  Then:  Both returned

TEST: recall --since filters in SQL (not post-query)
  Given: 10 shards, 3 created in last 7 days, 7 older
  When:  `cxp recall "query" --since 7d --limit 5`
...
TEST: shard update content
  ...
  When:  `cxp shard update <id> --body "Updated"`
  Then:  Content is "Updated", embedding regenerated

TEST: shard update title
  Given: Shard with title "Original Title"
  When:  `cxp shard update <id> --title "New Title"`
  Then:  Title changed

TEST: shard update knowledge shard warns
  Given: Shard with type=knowledge
  When:  `cxp shard update <id> --body "New content"`
  Then:  Warning about using cxp knowledge update, content still updated

TEST: shard close
  Given: Open shard
  When:  `cxp shard close <id>`
  Then:  Status is 'closed', updated_at refreshed

TEST: shard reopen
  Given: Closed shard
  When:  `cxp shard reopen <id>`
...
TEST: memory add with labels
  When:  `cxp memory add "Test memory" --label lesson-learned,pipeline`
  Then:  Shard has labels

TEST: memory add with references
  Given: Shard pf-bug-03 exists
  When:  `cxp memory add "Related to bug" --references pf-bug-03`
...
TEST: memory add with references to non-existent shard
  ...
  When:  `cxp memory add "Test" --references nonexistent`

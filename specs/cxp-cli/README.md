...
4. **Semantic search** — You can search shards by meaning, not just keywords. Shards
   are embedded on creation (pgvector). `cxp recall "deployment issues"` finds relevant
...
(blocked-by, child-of, implements, etc.) forming a graph. The `cxp` CLI is how you
interact with all of it.

**`cxp` is separate from `penf`.** `penf` handles Penfold-specific operations (email
pipeline, entities, acronyms). `cxp` is project-agnostic infrastructure — reusable
...
These specs define everything needed to build `cxp`. Each spec is self-contained: data
...
| [SPEC-1](SPEC-1-semantic-search.md) | Draft | pgvector embeddings, `cxp recall` for semantic search |
...
| [SPEC-5](SPEC-5-unified-search.md) | Draft | Unified `cxp recall`, shard CRUD, graph edges, labels |
| [SPEC-6](SPEC-6-hierarchical-memory.md) | Draft | Sub-memories, pointer blocks, access telemetry, `cxp memory tree` |
| [SPEC-7](SPEC-7-shard-lifecycle.md) | Draft | Epics, focus tracking, shard assign/close, `cxp shard next/board` |
...
- **On startup:** load root memories (`cxp memory list --roots`). Read their
  sub-memory pointer blocks to understand what detail is available.
- **When you need detail:** follow a pointer — `cxp memory show <child-id>`.
...
- **When you learn something new:** store it as a sub-memory under the right
  parent — `cxp memory add-sub <parent-id> --title "..." --body "..."`. The
...
- **Navigation:** `cxp memory tree` shows the full hierarchy. `cxp memory hot`
...
1. When you pick up a shard to implement: `cxp shard assign <id>`
2. When you finish: `cxp shard close <id> --reason "Done: ..."`
...
5. Check what to work on next: `cxp shard next`
6. See current state: `cxp shard board` or `cxp epic show <id>`
...
```bash
cxp focus set <epic-id>      # "I'm working on this epic"
cxp focus                    # show current epic + progress
cxp shard next               # next unblocked shard within focused epic
cxp shard board              # kanban view scoped to focused epic
```

...
1. **Standalone Go binary** — `cxp`, independent of `penf` and `palace`
2. **Project-scoped config** — `~/.cxp/config.yaml` (global) + `.cxp.yaml` (project)
...
### Project config: `.cxp.yaml` (project root, committed to repo)
...
```
CXP_HOST          Override connection host
CXP_DATABASE      Override database name
CXP_USER          Override database user
CXP_PROJECT       Override project name
CXP_AGENT         Override agent identity
```

### Precedence

Environment variables > `.cxp.yaml` (project) > `~/.cxp/config.yaml` (global) > defaults.
...
├── init                # Create .cxp.yaml in current directory
├── version             # CLI version
│
...
cxp/
├── main.go                     # Entry point
├── go.mod                      # Module: github.com/otherjamesbrown/context-palace/cxp
├── cmd/
│   ├── root.go                 # Root command, config loading, DB connection
│   ├── status.go               # cxp status
│   ├── init.go                 # cxp init
│   ├── memory.go               # cxp memory subcommands
│   ├── backlog.go              # cxp backlog subcommands
│   ├── message.go              # cxp message subcommands
│   ├── session.go              # cxp session subcommands
│   ├── context.go              # cxp context subcommands
│   ├── task.go                 # cxp task subcommands (from palace)
│   └── artifact.go             # cxp artifact subcommands (from palace)
└── internal/
    └── client/
...
2. **Init:** `cxp init` creates `.cxp.yaml` with project name. Detects project from
   git remote or directory name if possible.
3. **Status:** `cxp status` connects to DB and shows:
   ```
   Context Palace
     Host:     dev02.brown.chat
     Database: contextpalace
   ...
   ```
4. **Config precedence:** Env var > .cxp.yaml > ~/.cxp/config.yaml > defaults.
...
|------|-------------------|
| No `.cxp.yaml` in project | Fall back to `~/.cxp/config.yaml`. If no config at all: "Run `cxp init` to configure." |
...
| Config file syntax error | "Error parsing ~/.cxp/config.yaml: <yaml error>". |
| Both palace and cxp installed | No conflict. Different binaries, different config paths. |

## Migration from `palace`

The existing `palace` binary has `task` and `artifact` commands. These are absorbed
into `cxp` with identical behavior. `palace` can be deprecated after `cxp` is stable.

| `palace` command | `cxp` equivalent | Notes |
|------------------|----------------|-------|
| `palace task get <id>` | `cxp task get <id>` | Identical |
| `palace task claim <id>` | `cxp task claim <id>` | Identical |
| `palace task progress <id> <note>` | `cxp task progress <id> <note>` | Identical |
| `palace task close <id> <summary>` | `cxp task close <id> <summary>` | Identical |
| `palace artifact add <id> <type> <ref> <desc>` | `cxp artifact add <id> <type> <ref> <desc>` | Identical |

Config migration: `~/.palace.yaml` → `~/.cxp/config.yaml`. Different format but
same connection info. `cxp init --migrate-palace` reads old config.
...
TEST: LoadConfig with no files returns defaults
  Given: No ~/.cxp/config.yaml, no .cxp.yaml, no env vars
  When:  LoadConfig() is called
  Then:  Host = "dev02.brown.chat", Database = "contextpalace", Agent = ""
         Error: agent is required

TEST: LoadConfig reads global config
  Given: ~/.cxp/config.yaml with host=testhost, agent=test-agent
  When:  LoadConfig() is called
  Then:  Host = "testhost", Agent = "test-agent"

TEST: LoadConfig project overrides global
  Given: ~/.cxp/config.yaml with agent=global-agent
         .cxp.yaml with agent=project-agent
  When:  LoadConfig() is called
  Then:  Agent = "project-agent"

TEST: LoadConfig env overrides all
  Given: ~/.cxp/config.yaml with host=confighost
         CXP_HOST=envhost
  When:  LoadConfig() is called
  Then:  Host = "envhost"

TEST: LoadConfig walks up to find .cxp.yaml
  Given: .cxp.yaml exists in parent directory
         CWD is a subdirectory
  When:  LoadConfig() is called
  Then:  Project config found and loaded

TEST: LoadConfig with invalid YAML
  Given: ~/.cxp/config.yaml contains invalid YAML
  When:  LoadConfig() is called
...
TEST: LoadConfig migrate-palace
  ...
  Then:  Reads palace config, maps to cxp config format
```
...
```
TEST: cxp init creates .cxp.yaml
  Given: Empty directory, no .cxp.yaml
  When:  `cxp init --project test-project --agent test-agent` is run
  Then:  .cxp.yaml created with project=test-project, agent=test-agent

TEST: cxp init detects git project name
  Given: Git repo with remote origin=github.com/org/my-project.git
  When:  `cxp init` is run (no --project flag)
  Then:  .cxp.yaml created with project=my-project

TEST: cxp init refuses to overwrite
  Given: .cxp.yaml already exists
  When:  `cxp init` is run
...
```
TEST: cxp message send works
  Given: Valid config
  When:  `cxp message send agent-test "Test Subject" --body "Test body"` is run
  Then:  Message shard created, ID returned

TEST: cxp message inbox works
  Given: Message sent to current agent
  When:  `cxp message inbox` is run
  Then:  Message appears in output with subject, sender, date

TEST: cxp memory add works
  Given: Valid config
  When:  `cxp memory add "Test memory"` is run
  Then:  Memory shard created, ID returned

TEST: cxp memory list works
  Given: Memory shards exist
  When:  `cxp memory list` is run
  Then:  Lists memory shards with ID, date, content preview

TEST: cxp backlog add works
  Given: Valid config
  When:  `cxp backlog add "Test item" --priority high` is run
  Then:  Backlog shard created with priority=1

TEST: cxp task get works (palace parity)
  Given: Task shard exists with known ID
  When:  `cxp task get <id>` is run

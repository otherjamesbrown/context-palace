...
6. **`cxp knowledge` commands** — create, list, show, update, append, history, diff (7 commands)
...
|-------|------|----------|--------|-------------|
| `doc_type` | string | Yes (on create) | `cxp knowledge create --doc-type` | One of: architecture, vision, roadmap, decision, reference |
...
| `last_changed_by` | string | Yes | SQL functions | Identity of the agent/user who made the most recent change. Source: `agent_name` from `~/.cxp/config.yaml`, or `--agent` CLI flag if overridden. Falls back to `$USER` environment variable if neither is set. |
...
1. **WHO writes it?** `cxp knowledge create --doc-type <type>` (user/agent via CLI)
2. **WHEN is it written?** On creation only. Immutable after.
3. **WHERE is it stored?** `shards.metadata->>'doc_type'`
4. **WHO reads it?** `cxp knowledge list` (for display), `cxp knowledge list --doc-type <type>` (for filtering)
...
4. **WHO reads it?** `cxp knowledge show` (display), `cxp knowledge list` (display), `cxp knowledge history` (ordering), `cxp knowledge show --version N` (lookup)
...
4. **WHO reads it?** `cxp knowledge history` (display column), `cxp knowledge show` (display).
...
4. **WHO reads it?** `cxp knowledge history` (display column)
...
### `cxp knowledge create` — Create a knowledge document

```bash
# With inline body
cxp knowledge create "System Architecture" \
...
### `cxp knowledge list` — List knowledge documents
...
### `cxp knowledge show` — Display a knowledge document

```bash
# Show current version
cxp knowledge show pf-arch-001

# Show specific historical version
cxp knowledge show pf-arch-001 --version 2

# JSON output
cxp knowledge show pf-arch-001 -o json
```
...
### `cxp knowledge update` — Update document content (versioned)

```bash
cxp knowledge update pf-arch-001 \
...
### `cxp knowledge append` — Append content to document (versioned)

```bash
cxp knowledge append pf-dec-001 \
...
### `cxp knowledge history` — Show version history

```bash
cxp knowledge history pf-arch-001
cxp knowledge history pf-arch-001 -o json
```
...
### `cxp knowledge diff` — Diff between versions

```bash
# Diff current vs previous
cxp knowledge diff pf-arch-001

# Diff specific versions
cxp knowledge diff pf-arch-001 --from 1 --to 3

# JSON output
cxp knowledge diff pf-arch-001 -o json
```
...
```
cxp knowledge update <id> --body-file new.md --summary "..."
    │
    ▼
...
BEGIN
    ...
    IF current_status = 'closed' THEN
        RAISE EXCEPTION 'Knowledge document % is closed. Reopen with cxp shard reopen before updating.', p_shard_id;
    END IF;
...
BEGIN
    ...
    IF current_status = 'closed' THEN
        RAISE EXCEPTION 'Knowledge document % is closed. Reopen with cxp shard reopen before appending.', p_shard_id;
    END IF;
...
1. **Create:** `cxp knowledge create` creates shard with `type=knowledge`, `doc_type` in
   metadata, `version=1`, `last_changed_by` set. Returns shard ID.
2. **List:** `cxp knowledge list` shows all knowledge docs with doc_type, version, last
   updated date. `--doc-type` filter works. JSON output matches schema.
3. **Show:** `cxp knowledge show` displays full content, metadata, version info, labels.
...
12. **Closed doc protection:** `update_knowledge_doc()` and `append_knowledge_doc()` reject closed documents with a clear error. `cxp knowledge show` works on closed docs (read-only).
...
| Delete knowledge document | Use `cxp shard close <id>` (SPEC-5). Status set to closed. All version snapshots preserved. |
...
| `cxp shard update` on type=knowledge | Bypasses versioning — updates content directly without creating snapshot. This is intentional: `cxp shard update` is the low-level tool, `cxp knowledge update` is the versioned wrapper. |
| `cxp knowledge show` on closed document | Works — closed documents are still readable. Shows content with a "(closed)" indicator. |
| `cxp knowledge update` on closed document | Error: "Knowledge document pf-arch-001 is closed. Reopen with cxp shard reopen before updating." |
| `cxp knowledge append` on closed document | Error: "Knowledge document pf-arch-001 is closed. Reopen with cxp shard reopen before appending." |
| `cxp knowledge list` shows closed docs? | No — list defaults to `status = 'open'`. Use `cxp shard list --type knowledge --status closed` to find closed knowledge docs. |
...
| **SPEC-1** (semantic search) | Knowledge docs get embeddings on create/update. Version snapshots (closed) searchable via `cxp recall --include-closed`. Embedding regenerated by Go code after SQL function returns. |
...
| **SPEC-5** (unified search) | Knowledge docs appear in `cxp shard list --type knowledge`. `cxp shard close <id>` is the delete mechanism. `cxp shard update` on knowledge shards bypasses versioning (low-level tool). |
...
TEST: create knowledge document
  When:  `cxp knowledge create "Test Arch" --doc-type architecture --body "# Architecture"`
  Then:  Exit code 0, returns shard ID
         `cxp knowledge show <id>` shows content, doc_type=architecture, version=1

TEST: create with body-file
  Given: File /tmp/test-doc.md with "# Test Content"
  When:  `cxp knowledge create "Test Doc" --doc-type reference --body-file /tmp/test-doc.md`
  Then:  Exit code 0
         `cxp knowledge show <id>` shows content "# Test Content"

TEST: create with invalid doc-type
  When:  `cxp knowledge create "Test" --doc-type bogus --body "content"`
  Then:  Exit code 1, error mentions valid types

TEST: create without body
  When:  `cxp knowledge create "Test" --doc-type architecture`
...
TEST: list with doc-type filter
  ...
  When:  `cxp knowledge list --doc-type architecture`
...
TEST: update preserves version
  ...
  When:  `cxp knowledge update <id> --body "V2 content" --summary "Updated"`
  Then:  `cxp knowledge show <id>` shows "V2 content", version=2
         `cxp knowledge show <id> --version 1` shows "V1 content"

TEST: update without summary
  Given: Knowledge doc exists
  When:  `cxp knowledge update <id> --body "New content"`
  Then:  Exit code 1, error: "--summary required"

TEST: multiple updates preserve chain
  Given: Knowledge doc
  When:  Update 3 times
  Then:  `cxp knowledge history <id>` shows 4 versions (1 original + 3 updates)
         All versions accessible via --version flag

TEST: append adds to end
  Given: Knowledge doc with content "Line 1"
  When:  `cxp knowledge append <id> --body "Line 2" --summary "Added line"`
...
TEST: both agents can update
  ...
  Then:  `cxp knowledge history <id>` shows correct changed_by for each version

TEST: update identical content rejected
  Given: Knowledge doc with content "Same"
  When:  `cxp knowledge update <id> --body "Same" --summary "No change"`
  Then:  Exit code 1, error: "Content is identical"

TEST: show non-existent version
  Given: Knowledge doc at v2
  When:  `cxp knowledge show <id> --version 5`
  Then:  Exit code 1, error: "Version 5 not found. Document has 2 versions."

TEST: update closed document rejected
  Given: Knowledge doc exists, closed via `cxp shard close <id>`
  When:  `cxp knowledge update <id> --body "New" --summary "Change"`
  Then:  Exit code 1, error: "Knowledge document <id> is closed. Reopen with cxp shard reopen before updating."

TEST: append closed document rejected
  Given: Knowledge doc exists, closed via `cxp shard close <id>`
  When:  `cxp knowledge append <id> --body "More" --summary "Add"`
...
TEST: JSON output for all commands
  ...
  When:  `cxp knowledge list -o json`
  Then:  Valid JSON array matching schema
  When:  `cxp knowledge show <id> -o json`
  Then:  Valid JSON object matching schema
  When:  `cxp knowledge history <id> -o json`
  Then:  Valid JSON array matching schema
  When:  `cxp knowledge diff <id> -o json`

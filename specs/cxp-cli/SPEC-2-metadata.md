...
cxp shard metadata get <shard-id>
# Output: JSON object

# Get specific metadata field
cxp shard metadata get <shard-id> lifecycle_status
# Output: "draft"

# Get nested field
cxp shard metadata get <shard-id> test_coverage.unit
# Output: 0

# Set metadata field
cxp shard metadata set <shard-id> lifecycle_status approved

# Set nested field
cxp shard metadata set <shard-id> test_coverage.unit 5

# Set JSON value
cxp shard metadata set <shard-id> test_coverage '{"unit": 5, "integration": 2}'

# Delete metadata key
cxp shard metadata delete <shard-id> deprecated_field

# Query shards by metadata
cxp shard query --type requirement --meta "lifecycle_status=approved"
cxp shard query --type task --meta "implements_requirement=pf-req-01"
cxp shard query --meta "priority=1"

# Create shard with metadata
cxp shard create --type requirement --title "Test Req" \
...
3. **Merge semantics:** `cxp shard metadata set` merges keys, doesn't replace entire object.
4. **Nested paths:** `set test_coverage.unit 5` creates intermediate objects if needed.
5. **GIN queries:** `cxp shard query --meta "key=value"` uses index, not seq scan.
6. **create_shard updated:** Accepts metadata parameter.
7. **JSON output:** `cxp shard metadata get <id> -o json` returns valid JSON.
...
TEST: metadata set and get round-trip
  Given: Shard created with cxp shard create
  When:  `cxp shard metadata set <id> lifecycle_status approved`
         `cxp shard metadata get <id> lifecycle_status`
  Then:  Returns "approved"

TEST: metadata nested set and get
  Given: Shard exists
  When:  `cxp shard metadata set <id> test_coverage.unit 5`
         `cxp shard metadata get <id> test_coverage.unit`
  Then:  Returns "5"

TEST: metadata query returns matching shards
  Given: 3 requirement shards, 1 with lifecycle_status=approved
  When:  `cxp shard query --type requirement --meta "lifecycle_status=approved"`
  Then:  Returns exactly 1 shard

TEST: metadata create with --meta flag
  Given: Valid config
  When:  `cxp shard create --type requirement --title "Test" --meta '{"priority": 1}'`
         `cxp shard metadata get <new-id> priority`
  Then:  Returns "1"
